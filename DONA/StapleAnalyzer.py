...
class StapleAnalyzer(Analyzer):
    def __init__(self, design_file:Path) -> None:
        super(StapleAnalyzer, self).__init__(design_file)
        self.sandwich_addresses = []
        self.kinetic_trap_addresses = []
    
    def get_start_node(self) -> np.array:
    ...
    def construct_all_staples(self) -> list:
        ...
        sandwich_strands = []
        kinetic_traps = []
        fiveprime_termini = self.get_start_node()
        ...
        complete_staples =  list(zip(staples, sandwich_strands, kinetic_traps))
        self.staple_master_list = complete_staples
    ...
    def visualize_vulnerabilities(self, vulnerability_type:str) -> None:
        if vulnerability_type == 'ss':
            condition = 1
        elif vulnerability_type == 'kt':
            condition = 2
        elif vulnerability_type == 'both':
            condition = None
        else:
            raise Exception("vulnerability_type parameter must be 'ss', 'kt' or 'both'.")
        for staple in self.staple_master_list:
            if condition != None:
                if staple[condition]:
                    self.change_color(staple[0])
            else:
                if staple[1] or staple[2]:
                    self.change_color(staple[0])
...
if __name__ == '__main__':
    ...
    constructor = StapleAnalyzer(design_filepath)
